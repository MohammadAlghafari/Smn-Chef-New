...
import 'package:smn_admin/data/auth/model/user.dart' as userModel;
...
class _MobileVerificationBottomSheetWidgetState
  ...
  /* verifysPhone() async {
    ...
    final PhoneVerificationFailed _verifyFailed = (FirebaseAuthException e) {
      showToast(context: context, message: e.message.toString());
  ...
  verifyCode(String code) async {
    final verified = await Provider.of<AuthViewModel>(context, listen: false).verifyOTP(widget.phone, code);
    if (verified) {
